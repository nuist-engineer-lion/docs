...
先说bios，bios的自检也叫POST，会对硬件信息进行读取，并进行基本检测防止出现严重故障，但是他并不能完全排除故障（比如一般只检测前640k的内存，但是具体实现不同不能一概而论）。

再说uefi，bios之所以需要POST的一个重要原因是，bios不能存储电脑已有的硬件信息，所以每次上电都需要重新初始化并进行配置。但是在uefi中，我们可以存储设备信息（这就是为什么重置cmos，（一般“放电”时同时进行的）是有用的），不需要每次都读取一次。同时，各个设备的init操作也变成类似懒加载的形式，在需要时调用。所以，在实际的问题中，可能会出现正常开机通过UEFI甚至进入系统之后才被内存或者硬盘引起的黑屏。

![](../../assets/feishu/media/262ae1a4438493ea3451f866.png)
