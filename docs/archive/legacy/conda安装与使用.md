...
`numpy ipykernel ...`：这个是指在创建时就安装的一些python包，也可以省略，但建议每次都带上ipykernel包，这个是jupyter notebook的内核
...
在软件包管理的上下文中，如 pip、apt或conda，**"源"**通常指的是存储和分发软件包的服务器或服务。
...
你可以通过conda config --show channels来查看你目前的频道列表。如以下是我个人的配置：
...
opencv建议用conda-forge频道里的版本，可以用conda install -c conda-forge opencv来安装

**在vsc中使用conda虚拟环境**

**在py文件中使用指定环境运行**

![](../../assets/legacy/images/conda/media/image1.png)

注：如果在选择了虚拟环境的情况下，按Ctrl+Shift+`键打开终端的话，会自动激活相应虚拟环境。但最前面可能不会有(envName)的标识符。具体参见 **Activate Environments in Terminal Using Environment Variables**

**在ipynb文件中使用指定虚拟环境**

![](../../assets/legacy/images/conda/media/image2.png)
